...
describe Comment do
  ...
  it 'is invalid without a description' do
    expect(build_stubbed(:comment, description: nil)).to have(1).errors_on(:description)
  end
  ...
  it 'is invalid without an entry' do
    expect(build_stubbed(:comment, entry: nil)).to have(1).errors_on(:entry)
  end

  it 'is invalid if the description is longer than the maximum description length' do
    expect(build_stubbed(:entry, description: '0' * 401)).to have(1).errors_on(:description)
  end
